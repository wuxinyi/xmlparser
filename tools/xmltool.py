		...
		#recursive parse
			...
			if container_type == 'map':
				...
				self.dump_str2cppfile(cppfile, blank+4, 'ele_end   = child->LastChildElement("' + subnode.nodeName + '");')				
 				self.dump_str2cppfile(cppfile, blank+4, 'while(ele_begin)')
				self.dump_str2cppfile(cppfile, blank+4, '{')
				self.dump_str2cppfile(cppfile, blank+6, 'if (strcmp(ele_begin->Name(),"' +subnode.nodeName+'") != 0) {')
				self.dump_str2cppfile(cppfile, blank+8, 'ele_begin = ele_begin->NextSiblingElement();')
				self.dump_str2cppfile(cppfile, blank+8, 'continue;')
				...
				self.dump_str2cppfile(cppfile, blank+8, 'break;')	
				self.dump_str2cppfile(cppfile, blank+6, '}')					
				self.dump_str2cppfile(cppfile, blank+6, 'ele_begin = ele_begin->NextSiblingElement();')		
				self.dump_str2cppfile(cppfile, blank+4, '}')
				self.dump_str2cppfile(cppfile, blank+2, '}\n') 
   ...
   # parse nodes
		...
		#recursive parse
			...
			if container_type == 'map':
				...
				self.dump_str2cppfile(cppfile,blank+2,'typedef std::map<' + self.parse_label(container_key_type) + ',' + container_value_type + '>::iterator '+subnode.nodeName.upper() + '_It;')
				self.dump_str2cppfile(cppfile,blank+2,'typedef std::map<' + self.parse_label(container_key_type) + ',' + container_value_type + '>::const_iterator '+subnode.nodeName.upper() + '_ConstIt;\n')
			elif container_type == 'vector':
				container_value_type = 'struct ' + subnode.nodeName.upper()
				final_label = 'std::vector<' + container_value_type + '> ' + subnode.nodeName + ';'
				self.dump_str2cppfile(cppfile, blank+2, final_label)
				self.dump_str2cppfile(cppfile,blank+2, 'typedef std::vector<' + container_value_type + '>::iterator '+subnode.nodeName.upper() + '_It;')
				self.dump_str2cppfile(cppfile,blank+2, 'typedef std::vector<' + container_value_type + '>::const_iterator '+subnode.nodeName.upper() + '_ConstIt;\n')
			else:
				container_value_type = 'struct ' + subnode.nodeName.upper()
